...
//			pages[i] = MyPage.boolean TEST = false;
...
//		writeObjs(accessLogs, accessLogFileOut);newRanInst();
//			System.out.println(pages[i].toString());
//		}
		boolean TEST = false;
		if (args.length < 1) {
			System.out.println("Usage: java -jar  DataGenerator.jar [output_folder_path]");
			System.exit(1);
		}
		...
		System.out.println("outputing data at: " + PATH);
		
		String mypageFileOut = PATH + "mypage.csv";
		String friendsFileOut = PATH + "friends.csv";
		String accessLogFileOut = PATH + "access-log.csv";
		
		if (TEST) {
			AllConstants.RATIO = (float)1/(float)100;
			AllConstants.recalculate();
			mypageFileOut = PATH + "mypage_test.csv";
			friendsFileOut = PATH + "friends_test.csv";
			accessLogFileOut = PATH + "access-log_test.csv";
		} 
		System.out.println("Generating MyPage dataset");
		MyPage[] pages = MyPage.generateMyPages();
		writeObjs(pages, mypageFileOut);
